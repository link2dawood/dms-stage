...
// 		$value = isset( $data[ $key ] ) ? trim( $data[ $key ] ) : '';
		...
		$value = preg_replace('/\x{00A0}/u', '', $value); // remove nbsp
		$value = trim($value);
...
// Sanitize input
function csvipl_sanitize_value( $value ) {
    if ( is_string( $value ) ) {
        $value = preg_replace('/\x{00A0}/u', '', $value); // remove non-breaking spaces
...
// 				$new_val     = isset( $listingArr[0][ $csv_key ] ) ? $listingArr[0][ $csv_key ] : null;
				$new_val     = isset( $listingArr[0][ $csv_key ] ) ? 
					csvipl_sanitize_value( $listingArr[0][ $csv_key ] ) : null;
