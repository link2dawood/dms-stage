...
(function( $ ) {
    ...
    redux.field_objects.rev_slider = redux.field_objects.rev_slider || {};

    $( document ).ready(
        function() {
            redux.field_objects.rev_slider.init();
        }
    );

    redux.field_objects.rev_slider.init = function( selector ) {

        if ( !selector ) {
            selector = $( document ).find( ".redux-group-tab" ).find( '.redux-container-revolution_shortcode' );
        }
                ...
                el.find( 'div.redux-slider-container' ).each(
                    function() {
                        ...
                        if ( displayValue == DISPLAY_TEXT ) {
                            ...
                            redux.field_objects.rev_slider.loadSelect( defClassOne, minVal, maxVal, resVal, stepVal );

                            if ( handles === 2 ) {
                                redux.field_objects.rev_slider.loadSelect( defClassTwo, minVal, maxVal, resVal, stepVal );
                            }
    ...
    redux.field_objects.rev_slider.isFloat = function( mixed_var ) {
        return +mixed_var === mixed_var && (!(isFinite( mixed_var ))) || Boolean( (mixed_var % 1) );
    };

    // Return number of integers after the decimal point.
    redux.field_objects.rev_slider.decimalCount = function( res ) {
        var q = res.toString().split( '.' );
        return q[1].length;
    };

    redux.field_objects.rev_slider.loadSelect = function( myClass, min, max, res, step ) {
        ...
        for ( var i = min; i <= max; i = i + res ) {
            ...
            var n = i;
            if ( redux.field_objects.rev_slider.isFloat( res ) ) {
                var decCount = redux.field_objects.rev_slider.decimalCount( res );
