<?php /* Template Name: New Inventorypage Template */
...
function rangeFilters($title, $class) {
    $html = '<div class="inventory-filterbar__year-search inventory-filterbar__border-bottom px-2">'.
			...
			'<div class="d-flex align-items-center justify-content-center mb-3 font-lg" style="column-gap: 10px;">'.
			'<p class="'.$class.'-min-text font-weight-bold text-grey-3 font-segoe p-0">0</p>'.
			'<span class="range-text-divider text-grey-3">-</span>'.
            '<p class="'.$class.'-max-text font-weight-bold text-grey-3 font-segoe p-0">0</p>'.
			'</div>'.
			'<form class="inventory-filterbar__'.$class.' fake-'. ($class === 'price' ? 'price' : 'mileage') .' position-relative w-100 pb-3"></form>'.
            '<div class="d-flex align-items-center justify-content-between">'.
		'<p class="'.$class.'-min-text font-weight-bold text-grey-3 font-segoe p-0 font-lg">0</p>'.
            '<p class="'.$class.'-max-text font-weight-bold text-grey-3 font-segoe p-0 font-lg">0</p></div>'.
            '</div>
			</div>';
    return $html;
}
...
<style>
	...
	.bg-green {
		background-color: #00463c !important;
	}
	.inventory-content-wrapper__listings-wrapper .inventory-filterbar__searchbar-wrapper {
		border-bottom: unset !important;
	}
	.inventory-content-wrapper__listings-wrapper .inventory-filterbar__searchbar-wrapper input[type="search"] {
		font-weight: bold;
		color: gray;
		font-style: normal;
	}
	.inventory-filterbar__year:hover label, .inventory-filterbar__year input:checked + label {
		color: #00463c !important;
	}
...
<?php 
// 			echo divi_child_rankmath_breadcrumbs(); 
            $bannerGroups = array(
                'default' => array(
                    'field' => 'new_vehicles_banner_group',
                    'bigScreen' => 'new_vehicles_big_screen_laptops_banner',
                    'smallScreen' => 'new_vehicles_small_screen_laptops_banner',
                    'heading' => 'new_vehicles_banner_heading',
                    'content' => 'new_vehicles_banner_content',
                    'link' => 'new_vehicles_banner_link'
                )
            );
            ...
            $heroBannerHeading = '';
            $heroBannerContent = '';
            $isRepeater = false; // Flag to track if repeater is used

            foreach ($bannerGroups as $key => $bannerGroup) {
                if ($type == $key && $bannerGroupValue = get_field($bannerGroup['field'], 'options')) {
        
                    // Check if the field is a repeater and loop through it
                    if (have_rows($bannerGroup['field'], 'options')) {
                        $isRepeater = true; // Repeater found, set the flag
                        ...
                        while (have_rows($bannerGroup['field'], 'options')) : the_row();
                            ...
                            $link = get_sub_field($bannerGroup['link']); // Get the link field
                            ...
                            echo '<div>';
                            // Use <picture> tag to conditionally load the image based on screen size
                            echo '<a href="' . esc_url($link) . '" class="inventory-hero-slide-wrapper">'; // Apply the link
                            echo '<picture>';
                            ...
                            echo '</picture>';
                            echo '</a>'; // Close the link tag
                            echo '</div>';
        
                        endwhile;
                        echo '</div>'; // End Slick Slider
                    } else {
                        // Default behavior if it's not a repeater
            ...
            // Only display divi_child_page_banner if the field is NOT a repeater
            if (!$isRepeater) {
                echo divi_child_page_banner($heroThumbnail, $heroThumbnailAlt, '', '', $heroBannerHeading, $heroBannerContent, $heroThumbnailSmall);
            }
?>

	<div class="entry-content pt-lg-5 pb-3 pb-md-4 pb-lg-5 position-relative" style="z-index:1;">
		<div class="px-g">
			<div class="inventory-content-wrapper row">
				<div class="col-12 col-md-5 col-lg-3">
                    <div
						 class="<?php echo (!empty($stickyFilterbar) ? "filterbar-sticky" : "position-relative"); ?>"
						 style="z-index:111;">
                        <div class="inventory-filterbar">
							
                            <!-- mobile search bar -->
                            <?php
							echo createSearchBar('secondary-search-filters w-100 position-relative', 'd-flex d-md-none border-0 p-0', 'secondary-search-icon') ;
							echo vehiclesCount('mobile'); ?>
                            <!-- open mobile filterbar -->
							
                            <div
								 class="mt-20 mb-20 d-flex d-md-none justify-content-between flex-column align-items-start">
								<div class="d-flex justify-content-between align-items-center w-100">
                                    <div class="inventory-filterbar-toggle-wrapper inventory-filterbar-mobile-toggle d-flex align-items-center justify-content-start">
                                        <div class="inventory-filterbar__img-wrapper d-flex align-items-center">
                                            <img
												 src="<?php echo site_url();?>/wp-content/themes/divi-child/assets/images/inventory-filter-mobile.webp"
												 alt="inventory filter icon" />
                                        </div>
                                        <strong
												class="inventory-filterbar-toggle-text ml-2 text-capitalize text-green font-helvetica">filters/sort</strong>
                                    </div>
									<div 
										 class="inventory-products-bar__clear-selected-filters">
                                        <button 
												class="inventory-products-bar__clear-selected-filters-button font-weight-normal text-green">clear all</button>
                                    </div>
                                </div>
                            </div>
							
                            <!-- mobile searchbar ended -->
                            <div class="inventory-filters-main-wrapper inventory-filters-mobile-sidebar">
                                <div class="inventory-filters-mobilebar-heading d-md-none">
                                    ...
                                    <span class="inventory-filters-mobilebar-close inventory-filters-mobilebar-close-icon">
                                ...
                                <div class="inventory-filterbar__year-search inventory-filterbar__border-bottom d-lg-none">
                                    ...
                                    <form class="inventory-products-bar__sort-filter-form pl-0 sort expanding-section" >
                                        <?php echo sortOptions('secondary-sort-filter'); ?>
                                    </form>
                                </div>
								
								<?php $is_new_inventory = is_page( 'new-vehicles-durango-colorado' ); ?>
								
								<!-- Vehicle Type Filters -->
								<div class="vehicle-condition-filter-wrap">
									<div class="vehicle-condition-filter-block">
										<label for="vehicle-condition-filter-new">
											<?php echo esc_html( 'New' ); ?>
										</label>
										<input type="radio"
											   class="vehicle-condition-filter"
											   name="vehicle-condition-filter"
											   id="vehicle-condition-filter-new" checked value="new" />
									</div>
									<div class="vehicle-condition-filter-block">
										<label for="vehicle-condition-filter-certified">
											<?php echo esc_html( 'Certified' ); ?>
										</label>
										<input type="radio"
											   class="vehicle-condition-filter"
											   name="vehicle-condition-filter"
											   id="vehicle-condition-filter-certified"
											   value="certified" />
									</div>
									<div class="vehicle-condition-filter-block">
										<label for="vehicle-condition-filter-pre-owned">
											<?php echo esc_html( 'Pre-Owned' ); ?>
										</label>
										<input type="radio"
											   class="vehicle-condition-filter"
											   name="vehicle-condition-filter"
											   id="vehicle-condition-filter-pre-owned"
											   value="pre-owned" />
									</div>
								</div>
								
                                <?php
                                    echo checkboxOptionLoader('Make', 'fake-make');
                                    echo dmc_dropdown_filter_options( 'Model', 'fake-model' );
									echo checkboxOptionLoader('Year', 'fake-year');
									echo dmc_dropdown_filter_options( 'Trim', 'fake-trim' );
									echo rangeFilters('Price', 'price');
								?>
								
								<!-- Exterior color -->
                                <div class="inventory-filterbar__year-search inventory-filterbar__border-bottom px-2">
									<div 
										 class="inventory-filterbar__title d_flex d_flex__justify-between
												d_flex__align-center d_flex__wrap cursor-pointer">
										<h2 class="inventory-filterbar__heading font_bold
												   p_0 font_helvetica m_0 text-green">Color
										</h2>
									...
									<div class="expanding-section">
										<!-- Exterior Color -->
										<h4 class="text-grey-3 font-weight-bold font-lg pb-0 mb-3 font-segoe">
											<?php echo esc_html( 'Exterior' ); ?>
										</h4>
										<form class="inventory-filterbar__exteriorColor-search-wrapper
													 exteriorColor row fake-exterior-color w-100">      
										</form>
										
										<!-- Interior Color -->
										<h4 class="text-grey-3 font-weight-bold font-lg pb-0 mb-3 font-segoe">
											<?php echo esc_html( 'Interior' ); ?>
										</h4>
										<form class="inventory-filterbar__interior_color-search-wrapper
													 interior_color row fake-interior-color w-100"> 
										</form>
								...
								<?php
									// echo checkboxOptionLoader('Features', 'fake-features');
								 	echo checkboxOptionLoader('Engine', 'fake-engine');
    ...
    <div class="inventory-products-bar">
		...
		<?php
			echo createSearchBar('main-search-filters w-100 position-relative', null, 'main-search-icon');
		?>
        <div class="d-none d-lg-flex justify-content-between align-items-center">
            ...
            <div class="inventory-products-bar__meta-filters d_flex d_flex__justify-end w-100">
                ...
                <div class="inventory-products-bar__sort-filter d_flex">
                    ...
                    <form class="inventory-products-bar__sort-filter-form position-relative">
                        <?php  echo sortOptions('main-sort-filter'); ?>
                    </form>
                </div>
            </div>
        </div>
        <div class="inventory-products-bar__selected-filters-wrapper pt-3 d-none justify-content-start flex-wrap">
            <div class="inventory-products-bar__selected-filters d_flex__wrap d_flex d_flex__align-center">
                <?php ?>
        </div>     
...
<!-- custom inventory template ends -->
</div>
</article>

<?php echo divi_child_stickersPopup(); ?>

<!-- compare bottom tab started -->
    <?php
    ...
    <div class="compare-listings__wrapper position_fixed <?php echo count($user_compared_vehicles) > 0 && !is_null($user_compared_vehicles) ? 'compare-listings__wrapper-open' : '' ?>">
        <div class="compare-listings__header d_flex d_flex__align-center d_flex__justify-between mb-2 bg-primary">
            ...
            <span class="compare-listings-shrink text-primary cursor-pointer rounded-circle-px bg-white d-flex align-items-center justify-content-center mr-3" style="width: 25px; height: 25px;transition:all .3s;"><i class="fa-solid fa-arrow-down"></i></span>
				<span class="compare-listings__minimize text-primary cursor-pointer rounded-circle-px bg-white d-flex align-items-center justify-content-center mr-3" style="width:25px; height:25px;">
					<i class="fa-solid fa-compress" style="pointer-events:none;user-select:none;"></i>
<!-- 					<i class="fa fa-times" style="pointer-events:none;user-select:none;"></i> -->
				</span>
            <span class="compare-listings__close text-primary cursor-pointer rounded-circle-px bg-white d-flex align-items-center justify-content-center" style="width:25px; height:25px;">
                <i class="fa fa-times" style="pointer-events:none;user-select:none;"></i>
            </span>
            </div>
        </div>
        <div>
            <div>
                <div class="compare-listings__cta compare-btn d-flex d-md-none align-items-center flex-row py-2 w-75 mx-auto justify-content-between px-3">
                    <span class="compare-listings__remove-all mt-0">
                        Remove all
                    </span>
                    <a href="javascript:void(0)" class="compare-listings__compare <?php echo (count($user_compared_vehicles) < 2 ? 'disabled' : '') ?>">
                        Compare
                    </a>
                </div>
            </div>
                ...
                <?php
                    if (count($user_compared_vehicles) > 0) {
						$args = array(
							'post_type' => 'listings',
							'posts_per_page' => -1,
							'post__in' => $user_compared_vehicles
						);
						...
						if ($compareQuery->have_posts()) {
							...
							while ($compareQuery->have_posts()) {
								...
								$thumbnail = get_post_meta(get_the_ID(), 'gallery_images', true);
								$thumbnailImage = wp_get_attachment_url($thumbnail[0]);
								...
								echo '<div class="compare-listings__card" data-remove="' . get_the_ID() . '">' .
									'<div class="compare-listings__card-img">';
								if (!empty($image_url)) {
									echo '<img src="' . $image_url . '" loading="lazy" width="90" height="90"/>';
								}
								echo '</div>' .
									'<div class="compare-listings__card-remove">' .
									'<span class="fa fa-times" aria-hidden="true" data-remove="' . get_the_ID() . '"></span>' .
									'</div></div>';
							}

							// Close statement and connection after loop
							$stmt->close();
							$connection->close();
            ...
            <div class="compare-listings__cta compare-btn d-none d-md-flex">
                ...
                <span class="compare-listings__remove-all text-link">
                    Remove all
                </span>
            </div>
        </div>
...
<div class="inventory-page-paragraph">
    ...
    <div class="inventory-page-paragraph">
		<h2 style="text-align: center;">Didn't find what you're looking for?
        </h2>
		 <p style="text-align: center;"><span style="font-size:14px">At Durango Motor Company we want you to find the perfect vehicle, and we'll work hard to make sure you do. Simply tell us what you're looking for and when it's available you'll be the first to know!<span>
		 </p>
	</div>
...
<script>
    ...
    $('.slick-slider').slick({
        ...
        autoplaySpeed: 3000 // Set autoplay speed in milliseconds (3000ms = 3 seconds)
    });
